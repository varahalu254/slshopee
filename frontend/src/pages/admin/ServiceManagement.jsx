...
const ServiceManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingService
        ? `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/services/${editingService.id}`
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/services/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
